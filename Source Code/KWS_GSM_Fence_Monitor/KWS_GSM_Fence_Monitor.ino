...
char serv[20], client[45], top[35], dataToSend[100], buffer[6], imeiBuf[20];
...
unsigned long batinterval = 60000;
...
void sendDataMsg(String msgType)
{
  ...
  batt.toCharArray(buffer, 6);
  strcat(dataToSend, buffer);
  strcat(dataToSend, ",");

  ltoa(failures, buffer, 6);
  strcat(dataToSend, buffer);
  strcat(dataToSend, ",");

//  ltoa(freeMemory(), buffer, 6);
//  strcat(dataToSend, buffer);
//  strcat(dataToSend, ",");
